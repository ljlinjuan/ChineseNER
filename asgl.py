...
def _correlation_function(y_true, y_pred):
    return np.sum((y_true - np.mean(y_true)) * (y_pred - np.mean(y_pred))) / (np.std(y_true) * np.std(y_pred))
